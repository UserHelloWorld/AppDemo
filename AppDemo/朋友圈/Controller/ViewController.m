...
- (void)viewDidLoad {
    ...
    NSArray *contList = @[@"收到了福克斯李开复打开理论上的开始卡死了凯迪拉克",@"看的撒狂蜂浪蝶使肌肤的教科书的金卡的开始的健康卡将就暗示的；绿卡的骄傲风的季节阿凡达卡就卡奋达科技阿娇；士大夫快捷键快递费刷卡机发大水会计法就看看剪短发科技大厦就卡的技卡打卡机可敬的发送卡垃圾啊东方斯卡拉房间爱上了的咖啡机",@"似懂非懂开放式拉快递费绿山咖啡",@"灯数可拉开了快递费是考虑到萨洛克克里斯科勒卡戴珊可浪费大反馈大数据时代风力发电空间说拉德芳斯发大水了健康卡立方时代峻峰分手的距离分手快乐打开方式达克赛德克拉克拉放大看了房卡洛斯双方都开发就烦死了肯德基开发商的刻录机方式大家"];
    for (int i = 0; i < 4; i++) {
        NFModel *m = [[NFModel alloc] init];
        m.content = contList[i];
        m.userName = @"我试试";
        m.imageCount = arc4random_uniform(9);
       
        NSLog(@"%f",m.imageCount);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    NFModel *m = self.dataArray[indexPath.row];
    return m.rowHeight;
}
